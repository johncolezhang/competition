...
public class KMeans {
	...
	private ArrayList<Order> dataSet;// 数据集链表
	private ArrayList<Order> center;// 中心链表
	private ArrayList<ArrayList<Order>> cluster; // 簇
	...
	public ArrayList<ArrayList<Order>> getCluster() {
		return cluster;
	}
	 ...
	 * @return 中心点集
	 */
	private ArrayList<Order> initCenters() {
		ArrayList<Order> center = new ArrayList<Order>();
	 ...
	 */
	private ArrayList<ArrayList<Order>> initCluster() {
		ArrayList<ArrayList<Order>> cluster = new ArrayList<>();
		for (int i = 0; i < k; i++) {
			cluster.add(new ArrayList<Order>());
		}
	...
	private void clusterSet() {
		...
		for (int i = 0; i < dataSetLength; i++) {
			Node node = ServiceData.localPacageMaps.get(dataSet.get(i).dest_id);
			for (int j = 0; j < k; j++) {
				Node node2 = ServiceData.localPacageMaps
						.get(dataSet.get(j).dest_id);
				distance[j] = Rule.distanceTime(node, node2);
	 ...
	 * @return 误差平方
	 */
	private float errorSquare(Order element, Order center) {
		// float x = element[0] - center[0];
		// float y = element[1] - center[1];

		Node node = ServiceData.localPacageMaps.get(element.dest_id);
		Node node2 = ServiceData.localPacageMaps.get(center.dest_id);
		int errSquare = Rule.distanceTime(node, node2);
