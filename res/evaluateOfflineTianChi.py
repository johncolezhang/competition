...
# ------------- loading solution ------------
solution = {}
csvfile = file('result.csv','rU') # please input your solution path
lines = csv.reader(csvfile)
for line in lines:
		...
		for rInd in solution[dInd]:
			...
			if penalty_serv>0 or penalty_rout>0:
				...
				if tmp_cheat>C_cheat:
					print nInd[4],'error: do not cheat!'
			...
			#penalty_tw = 0 if nInd[3]>0 and orders[nInd[4]][5]==2 else  int(max(nInd[1]-orders[nInd[4]][3],0))*C_penalty_tw
			tmp_cost = serv_time+time_dist + penalty_rout + penalty_serv+penalty_tw	
                        if penalty_tw > 0 and nInd[4]=='F0331':
                            print nInd , penalty_tw
